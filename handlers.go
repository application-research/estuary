...
func (s *Server) ServeAPI() error {
	...
	user := e.Group("", s.AuthRequired(util.PermLevelUser))
	user.GET("/user/api-keys", withUser(s.handleUserGetApiKeys))
	user.POST("/user/api-keys", withUser(s.handleUserCreateApiKey))
	user.DELETE("/user/api-keys/:key", withUser(s.handleUserRevokeApiKey))
	user.GET("/user/export", withUser(s.handleUserExportData))
	user.PUT("/user/password", withUser(s.handleUserChangePassword))
	user.PUT("/user/address", withUser(s.handleUserChangeAddress))
	user.GET("/user/stats", withUser(s.handleGetUserStats))

	user.POST("/user/miner/claim", withUser(s.handleUserClaimMiner))
	user.GET("/user/miner/claim/:miner", withUser(s.handleUserGetClaimMinerMsg))
	user.POST("/user/miner/suspend/:miner", withUser(s.handleSuspendMiner))
	user.PUT("/user/miner/unsuspend/:miner", withUser(s.handleUnsuspendMiner))
	user.PUT("/user/miner/set-info/:miner", withUser(s.handleMinersSetInfo))

	uploads := e.Group("", s.AuthRequired(util.PermLevelUpload))
	uploads.POST("/content/add", withUser(s.handleAdd))
	uploads.POST("/content/add-ipfs", withUser(s.handleAddIpfs))
	uploads.POST("/content/add-car", util.WithContentLengthCheck(withUser(s.handleAddCar)))
	uploads.POST("/content/create", withUser(s.handleCreateContent))

	content := e.Group("", s.AuthRequired(util.PermLevelUser))
	content.GET("/content/by-cid/:cid", s.handleGetContentByCid)
	content.GET("/content/stats", withUser(s.handleStats))
	content.GET("/content/ensure-replication/:datacid", s.handleEnsureReplication)
	content.GET("/content/status/:id", withUser(s.handleContentStatus))
	content.GET("/content/list", withUser(s.handleListContent))
	content.GET("/content/deals", withUser(s.handleListContentWithDeals))
	content.GET("/content/failures/:content", withUser(s.handleGetContentFailures))
	content.GET("/content/bw-usage/:content", withUser(s.handleGetContentBandwidth))
	content.GET("/content/staging-zones", withUser(s.handleGetStagingZoneForUser))
	content.GET("/content/aggregated/:content", withUser(s.handleGetAggregatedForContent))
	content.GET("/content/all-deals", withUser(s.handleGetAllDealsForUser))
	...
	deals := e.Group("", s.AuthRequired(util.PermLevelUser))
	deals.GET("/deals/status/:deal", withUser(s.handleGetDealStatus))
	deals.GET("/deals/status-by-proposal/:propcid", withUser(s.handleGetDealStatusByPropCid))
	deals.GET("/deals/query/:miner", s.handleQueryAsk)
	deals.POST("/deals/make/:miner", withUser(s.handleMakeDeal))
	//deals.POST("/transfer/start/:miner/:propcid/:datacid", s.handleTransferStart)
	deals.GET("/deals/transfer/status/:id", s.handleTransferStatusByID)
	deals.POST("/deals/transfer/status", s.handleTransferStatus)
	deals.GET("/deals/transfer/in-progress", s.handleTransferInProgress)
	deals.GET("/deals/status/:miner/:propcid", s.handleDealStatus)
	deals.POST("/deals/estimate", s.handleEstimateDealCost)
	deals.GET("/deals/proposal/:propcid", s.handleGetProposal)
	deals.GET("/deals/info/:dealid", s.handleGetDealInfo)
	deals.GET("/deals/failures", withUser(s.handleStorageFailures))

	cols := e.Group("", s.AuthRequired(util.PermLevelUser))
	cols.GET("/collections", withUser(s.handleListCollections))
	cols.POST("/collections", withUser(s.handleCreateCollection))
	cols.DELETE("/collections/:coluuid", withUser(s.handleDeleteCollection))
	cols.POST("/collections/:coluuid", withUser(s.handleAddContentsToCollection))
	cols.GET("/collections/:coluuid", withUser(s.handleGetCollectionContents))
	cols.POST("/collections/:coluuid/commit", withUser(s.handleCommitCollection))

	e.POST("/fs/add", withUser(s.handleColfsAdd))

	pinning := e.Group("", openApiMiddleware, s.AuthRequired(util.PermLevelUser))
	pinning.GET("/pinning/pins", withUser(s.handleListPins))
	pinning.POST("/pinning/pins", withUser(s.handleAddPin))
	pinning.GET("/pinning/pins/:pinid", withUser(s.handleGetPin))
	pinning.POST("/pinning/pins/:pinid", withUser(s.handleReplacePin))
	pinning.DELETE("/pinning/pins/:pinid", withUser(s.handleDeletePin))

	// explicitly public, for now
	e.GET("/public/stats", s.handlePublicStats)
	e.GET("/public/by-cid/:cid", s.handleGetContentByCid)
	e.GET("/public/deals/failures", s.handlePublicStorageFailures)
	e.GET("/public/info", s.handleGetPublicNodeInfo)
	e.GET("/public/miners", s.handlePublicGetMinerStats)

	e.GET("/public/metrics/deals-on-chain", s.handleMetricsDealOnChain)

	e.GET("/public/net/peers", s.handleNetPeers)
	e.GET("/public/net/addrs", s.handleNetAddrs)

	e.GET("/public/miners", s.handleAdminGetMiners)
	e.GET("/public/miners/failures/:miner", s.handleGetMinerFailures)
	e.GET("/public/miners/deals/:miner", s.handleGetMinerDeals)
	e.GET("/public/miners/stats/:miner", s.handleGetMinerStats)
	e.GET("/public/miners/storage/query/:miner", s.handleQueryAsk)

	admin := e.Group("", s.AuthRequired(util.PermLevelAdmin))
	admin.GET("/admin/balance", s.handleAdminBalance)
	admin.GET("/admin/dealstats", s.handleDealStats)
	admin.GET("/admin/disk-info", s.handleDiskSpaceCheck)
	admin.GET("/admin/stats", s.handleAdminStats)
	admin.GET("/admin/system/config", withUser(s.handleGetSystemConfig))
	admin.POST("/admin/add-escrow/:amt", s.handleAdminAddEscrow)

	// miners
	admin.POST("/admin/miners/add/:miner", s.handleAdminAddMiner)
	admin.POST("/admin/miners/rm/:miner", s.handleAdminRemoveMiner)
	admin.POST("/admin/miners/suspend/:miner", withUser(s.handleSuspendMiner))
	admin.PUT("/admin/miners/unsuspend/:miner", withUser(s.handleUnsuspendMiner))
	admin.PUT("/admin/miners/set-info/:miner", withUser(s.handleMinersSetInfo))
	admin.GET("/admin/miners", s.handleAdminGetMiners)
	admin.GET("/admin/miners/stats", s.handleAdminGetMinerStats)
	admin.GET("/admin/miners/transfers/:miner", s.handleMinerTransferDiagnostics)

	admin.GET("/admin/cm/progress", s.handleAdminGetProgress)
	admin.GET("/admin/cm/all-deals", s.handleDebugGetAllDeals)
	admin.GET("/admin/cm/read/:content", s.handleReadLocalContent)
	admin.GET("/admin/cm/staging/all", s.handleAdminGetStagingZones)
	admin.GET("/admin/cm/offload/candidates", s.handleGetOffloadingCandidates)
	admin.POST("/admin/cm/offload/:content", s.handleOffloadContent)
	admin.POST("/admin/cm/offload/collect", s.handleRunOffloadingCollection)
	admin.GET("/admin/cm/refresh/:content", s.handleRefreshContent)
	admin.POST("/admin/cm/gc", s.handleRunGc)
	admin.POST("/admin/cm/move", s.handleMoveContent)
	admin.GET("/admin/cm/buckets", s.handleGetBucketDiag)
	admin.GET("/admin/cm/health/:id", s.handleContentHealthCheck)
	admin.GET("/admin/cm/health-by-cid/:cid", s.handleContentHealthCheckByCid)
	admin.POST("/admin/cm/dealmaking", s.handleSetDealMaking)
	admin.POST("/admin/cm/break-aggregate/:content", s.handleAdminBreakAggregate)
	admin.POST("/admin/cm/transfer/restart/:chanid", s.handleTransferRestart)
	admin.POST("/admin/cm/repinall/:shuttle", s.handleShuttleRepinAll)

	//	peering
	admin.POST("/admin/peering/peers", s.handlePeeringPeersAdd)
	admin.DELETE("/admin/peering/peers", s.handlePeeringPeersRemove)
	admin.GET("/admin/peering/peers", s.handlePeeringPeersList)
	admin.POST("/admin/peering/start", s.handlePeeringStart)
	admin.POST("/admin/peering/stop", s.handlePeeringStop)
	admin.GET("/admin/peering/status", s.handlePeeringStatus)

	admin.GET("/admin/net/peers", s.handleNetPeers)

	admin.GET("/admin/retrieval/querytest/:content", s.handleRetrievalCheck)
	admin.GET("/admin/retrieval/stats", s.handleGetRetrievalInfo)

	admin.POST("/admin/invite/:code", withUser(s.handleAdminCreateInvite))
	admin.GET("/admin/invites", s.handleAdminGetInvites)

	admin.GET("/admin/fixdeals", s.handleFixupDeals)
	admin.POST("/admin/loglevel", s.handleLogLevel)

	admin.GET("/admin/users", s.handleAdminGetUsers)

	admin.POST("/admin/shuttle/init", s.handleShuttleInit)
	admin.GET("/admin/shuttle/list", s.handleShuttleList)
