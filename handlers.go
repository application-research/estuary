...
// @Tags         buckets
// @Param        coluuid  query  string  true  "Bucket ID"
...
func (s *Server) handleColfsAdd(c echo.Context, u *util.User) error {
	...
	var col buckets.Bucket
	if err := s.DB.First(&col, "uuid = ?", coluuid).Error; err != nil {
		if xerrors.Is(err, gorm.ErrRecordNotFound) {
			return &util.HttpError{
				Code:    http.StatusNotFound,
				Reason:  util.ERR_RECORD_NOT_FOUND,
				Details: fmt.Sprintf("bucket: %s was not found", coluuid),
			}
		}
		return err
	}

	if err := util.IsBucketOwner(u.ID, col.UserID); err != nil {
		return err
	}
	...
	if err := s.DB.First(&content, "id = ?", contid).Error; err != nil {
		if xerrors.Is(err, gorm.ErrRecordNotFound) {
			return &util.HttpError{
				Code:    http.StatusNotFound,
				Reason:  util.ERR_RECORD_NOT_FOUND,
				Details: fmt.Sprintf("bucket content: %s was not found", contid),
			}
		}
		return err
	}
	...
	if err := s.DB.Create(&buckets.BucketRef{Bucket: col.ID, Content: content.ID, Path: path}).Error; err != nil {
		return errors.Wrap(err, "failed to add content to requested bucket")
	}
	return c.JSON(http.StatusOK, map[string]string{})
}
