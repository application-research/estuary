...
func (cm *ContentManager) PinContent(ctx context.Context, user uint, obj cid.Cid, filename string, cols []*collections.CollectionRef, origins []*peer.AddrInfo, replaceID uint, meta map[string]interface{}, makeDeal bool) (*types.IpfsPinStatusResponse, *operation.PinningOperation, error) {
	...
	// get user from database (for AddContentToCollection)
	var u *util.User
	if err := cm.db.First(&u, "id = ?", user).Error; err != nil {
		return nil, nil, err
	}

	if len(cols) > 0 {
		for _, c := range cols {
			// TODO(gabe): set overwrite flag when adding pin instead of always
			// 'true'
			err := collections.AddContentToCollection(string(c.Collection), string(cont.ID), *c.Path, true, cm.db, u)
			if err != nil {
				return nil, nil, err
			}
		}
