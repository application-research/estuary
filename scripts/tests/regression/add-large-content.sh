...
yes "this is for a large file" | head -n 10000000 > files/large-file
# Generate a large file.

set -x
curl --progress-bar -X POST $EST_HOST/content/add -H "Authorization: Bearer $ESTUARY_TOKEN" -H "Accept: application/json" -H "Content-Type: multipart/form-data" -F "data=@$EST_SAMPLE_LARGE_FILE"

# Convert it back to a small file
yes "this is for a large file" | head -n 100 > files/large-file